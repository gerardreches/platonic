...
class RouteServiceProvider extends ServiceProvider
{
	 ...
	 * @var string
	 */
	protected $namespace = 'Platonic\Blog\Http\Controllers';
    ...
    protected function mapModuleRoutes(Router $router)
    {
        $router->group(
			...
			function($router) {
				require (config('modules.path').'/Blog/Http/Routes/module.php');
			}
		);
    }
    ...
    protected function mapDashboardRoutes(Router $router)
    {
        $router->group(
			...
			function($router) {
				require (config('modules.path').'/Blog/Http/Routes/dashboard.php');
			}
		);
    }
}
